...
/** @brief Vide completement le plateau.
 * @param board Plateau a reinitialiser. */
void empty(Board *board);
/** @brief Retourne la piece presente sur une case.
 * @param board Plateau a consulter. 
 * @param i Indice de ligne. 
 * @param j Indice de colonne. 
 * @return Piece contenue sur la case demandee. */
Piece get_square(Board *board, int i, int j);
/** @brief Place une piece sur une case du plateau. 
 * @param board Plateau a modifier. 
 * @param i Indice de ligne. 
 * @param j Indice de colonne. 
 * @param p Piece a placer. */
void set_square(Board *board, int i, int j, Piece p);
/** @brief Place les pieces sur la position initiale. 
 * @param board Plateau a initialiser. */
void start(Board *board);
/** @brief Deplace une piece d'une case vers une autre.
 * @param board Plateau a modifier. 
 * @param i1 Ligne de depart.
 * @param j1 Colonne de depart.
 * @param i2 Ligne d'arrivee.
 * @param j2 Colonne d'arrivee. */
void move_piece(Board *board, int i1, int j1, int i2, int j2);
/** @brief Convertit une piece en caractere Unicode d'affichage.
 * @param p Piece a convertir.
 * @return Chaine representant la piece. */
const char *piece_to_char(Piece p);

/** @brief Indique si une piece est blanche. 
 * @param p Piece a tester. 
 * @return true si la piece est blanche. */
bool is_white(Piece p);
/** @brief Indique si une piece est noire. 
 * @param p Piece a tester. 
 * @return true si la piece est noire. */
bool is_black(Piece p);
/** @brief Indique si deux pieces sont adverses. 
 * @param attacker Piece qui attaque. 
 * @param target Piece cible. 
 * @return true si la cible est une piece adverse. */
